"""Resolve report files and regenerate when missing (ephemeral /tmp or production host disk)."""
import os
...
def regenerate_report_for_log(log: ReportLog) -> dict:
    ...
    family = FamilyDetails.query.filter_by(assessment_id=log.assessment_id).first()
    children_map = {c.id: c.child_number for c in family.children} if family else {}
    result = generate_report(
        str(log.assessment_id), calc, personal, comm, goals,
