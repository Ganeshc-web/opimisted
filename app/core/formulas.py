...
LIFE_EXPECTANCY = 80
PF_CONTRIBUTION_YEARLY_INCREASE = 0.05
...
def child_insurance_goal_name(child_number: int | None, goal_type: str) -> str | None:
    """Map flow4 child goal to Insurance sheet row key (e.g. Child 1 Graduation)."""
    if not child_number:
        return None
    short = _CHILD_INSURANCE_LABELS.get(goal_type)
    if short:
        return f"Child {child_number} {short}"
    # Custom Other child goals keep the entered name after "Child N".
    label = (goal_type or "").strip()
    if not label:
        return None
    return f"Child {child_number} {label}"
...
def build_insurance_rows(
    ...
    # Child goals keyed as "Child N <label>" (Graduation/PG/Marriage or custom Other name).
    child_goal_re = re.compile(r"^Child\s+(\d+)\s+(.+)$", re.IGNORECASE)
    child_goal_order = {"Graduation": 0, "PG": 1, "Marriage": 2}

    def _normalize_suffix(raw: str) -> str:
        if raw.lower() == "pg":
            return "PG"
        if raw.lower() == "graduation":
            return "Graduation"
        if raw.lower() == "marriage":
            return "Marriage"
        return raw.strip()

    def _child_sort_key(goal: dict) -> tuple:
        parsed = child_goal_re.match(str(goal.get("Goal") or ""))
        if not parsed:
            return (9999, 99, "")
        suffix = _normalize_suffix(parsed.group(2))
        return (
            int(parsed.group(1)),
            child_goal_order.get(suffix, 50),
            suffix.lower(),
        )

    for goal in sorted(
        (g for g in goal_results if child_goal_re.match(str(g.get("Goal") or ""))),
        key=_child_sort_key,
    ):
        label = str(goal.get("Goal"))
        parsed = child_goal_re.match(label)
        if parsed:
            short = _normalize_suffix(parsed.group(2))
            label = f"Child {int(parsed.group(1))} {short}"
        amount = display_amount(goal["future_cost"])
        if amount <= 0:
            continue
        rows.append(
            {
                "Need": label,
                "Years": int(goal["years_from_now"]),
                "Amount (₹)": amount,
                "Type": "Future value",
                "PV (₹)": insurance_pv(
                    int(goal["years_from_now"]),
                    amount,
                    "future",
                    real_rate_annual,
                    roi_post,
