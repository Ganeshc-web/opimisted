...
class TourDestinationsForBudget(Resource):
    ...
    def get(self):
        ...
        if category:
            query = query.filter(TourDestination.category == category)
        ...
        # If nothing sits in the band, return nearest destinations so the UI
        # always has a searchable list instead of a single default location.
        if pagination.total == 0:
            fallback = TourDestination.query
            if category:
                fallback = fallback.filter(TourDestination.category == category)
            pagination = (
                fallback.order_by(db.func.abs(TourDestination.budget_inr - budget))
                .paginate(page=page, per_page=per_page, error_out=False)
            )
...
class TourDestinationsList(Resource):
    @require_api_key
    @ns.doc(
        security="apikey",
        description=(
            "List all tour destinations for dropdowns. Optional category and "
            "country search filters. Use this when the UI needs the full list "
            "instead of budget-only reverse lookup."
        ),
    )
    @ns.param("category", "Optional destination category filter.", type=str, required=False, _in="query")
    ...
    def get(self):
        ...
        if category:
            query = query.filter(TourDestination.category == category)
        if country:
            query = query.filter(TourDestination.country.ilike(f"%{country}%"))

        pagination = (
            query.order_by(TourDestination.budget_inr.asc(), TourDestination.country.asc())
            .paginate(page=page, per_page=per_page, error_out=False)
        )
        return success_response(
            {
                "items": [serialize_destination(row) for row in pagination.items],
                "total": pagination.total,
                "total_pages": pagination.pages,
                "page": pagination.page,
                "per_page": pagination.per_page,
