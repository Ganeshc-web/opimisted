...
class EducationOptionsForBudget(Resource):
    ...
    @ns.doc(
        security="apikey",
        description=(
            "Reverse lookup from a target education budget to matching "
            "program/course/country combinations. Results are constrained "
            "by tolerance percentage and sorted by closest cost match."
        ),
    )
    @ns.param("budget", "Target budget in INR.", type=float, required=True, _in="query", example=9000000)
    @ns.param("level", "Optional education level filter.", type=str, required=False, _in="query", example="Post Graduation")
    ...
    def get(self):
        ...
        return success_response(
            {
                "items": [
                    serialize_program(program) for program in pagination.items
                ],
