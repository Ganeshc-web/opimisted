...
REQUEST_DEFAULTS = {
    # Zeros when FE omits retirement inputs — do not invent sample planning figures.
    "client_epf_annual": 0,
    "client_epf_accum": 0,
    "client_annual_ret_reqd": 0,
    "spouse_epf_annual": 0,
    "spouse_epf_accum": 0,
...
def get_body(payload=None):
    if payload is None:
        payload = request.get_json(silent=True) or {}
    body = {}
    for key, default in REQUEST_DEFAULTS.items():
        value = payload.get(key, default)
        body[key] = default if value is None else value
    return body
...
def goals_to_formula_input(goals, children_map: dict | None = None):
    children_map = children_map or {}
    rows = []
    for goal in goals:
        goal_name = goal.goal_type
        if goal.category == "child_goal" and goal.child_id:
            mapped = child_insurance_goal_name(
                children_map.get(goal.child_id), goal.goal_type
            )
            if mapped:
                goal_name = mapped
        rows.append(
...
def execute_calculation(assessment_id, body=None):
    """Run and persist calculation for an assessment. Used by API and Excel import."""
    assessment_id = (
        assessment_id
        if hasattr(assessment_id, "hex")
        else parse_uuid_param(str(assessment_id), "assessment_id")
    )
    ...
    try:
        ...
        children_map = (
            {c.id: c.child_number for c in family.children} if family else {}
        )
        goal_results, total_goals_monthly_sip = compute_all_goals(
            goals_to_formula_input(goal_rows, children_map),
            monthly_eff_pre,
        )
    ...
    output = CalculationOutput(
        ...
        client_provisions_made=client_calc["total_existing_provision"],
        ...
        spouse_provisions_made=spouse_calc["total_existing_provision"],
        ...
        household_monthly=body["household_monthly"],
    ...
    return {
        ...
        "investment_summary": build_investment_summary(
            assessment_id, calc=output, personal=personal
        ),
    }
...
class CalculateAssessment(Resource):
    ...
    def post(self, assessment_id):
        ...
        response_data = execute_calculation(assessment_id, get_body())
